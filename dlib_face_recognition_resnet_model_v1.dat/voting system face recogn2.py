...
def average_encoding(images):
    ...
    for frame in images:
        ...
        for (x, y, w, h) in faces:
            face = dlib.rectangle(int(x), int(y), int(x + w), int(y + h))  # FIXED
...
def match_face(encoding):
    for aadhaar, info in face_db.items():
        ...
        if np.linalg.norm(stored_enc - encoding) < 0.6:
            return aadhaar
    return None
...
def start_voting():
    ...
    if encoding is not None:
        ...
        if aadhaar:
            ...
            tk.Label(vote_window, text=f"Welcome {voter_name}!").pack(pady=10)
